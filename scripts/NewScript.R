...
WatershedMap <- leaflet() %>%
  ...
  addPolygons(data = islcoast, color = "black", weight = 1, fillOpacity = 0) %>%
  addPolylines(data = MCW, color = "red", weight = 2, fillColor = NA) %>%
  addPolylines(data = watersheds, color = "black", weight = 0.5) %>%
  addPolygons(data = watershed_unnamed, color = "#CC66CC", opacity = 0.05) %>%
  addPolygons(data = watershed_natural, color = "#FFCC00", opacity = 0.05) %>%
